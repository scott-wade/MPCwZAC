...
function hexrotor_dynamics(model::NamedTuple,x,u, wind)
    ...
    for i= 1:6
        α= α * (-1)^(i+1) # every other propeller is rotated opposite (starting with propeller one tilted +α)
        β= β * (-1)^(i+1) # every other propeller is rotated opposite (starting with propeller one tilted +β)
    ...
    for i=1:6
        thrust_propeller[i, 3] = kf*(u[i])
        #thrust_propeller[i, 3] = kf*(u[i]^2)
    end
    ...
    drag_propeller = zeros(eltype(u), 6, 3)
    for i=1:6
        drag_propeller[i, 3] = ((-1)^(i))* km*(u[i])
        #drag_propeller[i, 3] =  km*(u[i]^2)
    end
    ...
    torque_drag_body= sum(torque_drag_body, dims=1)
    ...
    # wind is a 3,1 vector

    
#   # xdot 
    [  
        v #+ wind_accel
        (f/mass) + wind_accel #wind_accel # wind modeled as an added acceleration on the current state
        ((1+norm(p)^2)/6) *(   I + 2*(skew(p)^2 + skew(p))/(1+norm(p)^2)   )*ω #(Q* skew(ω))'
        J\(τ - cross(ω,J*ω))
    ]
    # [  
    #     v
    #     (f/mass) + wind_accel # wind modeled as an added acceleration on the current state
    #     ((1+norm(p)^2)/6) *(   I + 2*(skew(p)^2 + skew(p))/(1+norm(p)^2)   )*ω #(Q* skew(ω))'
    #     -inv(J)* cross(ω, J*ω) + inv(J)* τ
    # ]
...
# rot_inertia= [(1/m)*Q zeros(3,3); zeros(3,3) J]
# matrix_form= [f; τ]

# decoupling_matrix= rot_inertia * matrix_form
...
function rk4(model,ode,x,u,wind,dt)
    # rk4 for discretizing
    k1 = dt*ode(model,x, u, wind)
    k2 = dt*ode(model,x + k1/2, u, wind)
    k3 = dt*ode(model,x + k2/2, u, wind)
    k4 = dt*ode(model,x + k3, u, wind)
...
function animate_hexrotor(Xsim, dt)
    ...
    mc.setobject!(vis[:drone][:base], robot_obj, mc.MeshPhongMaterial(color=mc.RGBA(1.5, 0.5, 0.8, 0.8)))
    #mc.settransform!(vis[:drone][:base], mc.Translation([0,0,0]) ∘ mc.LinearMap(0.001 * I))
    mc.settransform!(vis[:drone][:base], mc.Translation([0,0,0]) ∘ mc.LinearMap(0.0006 * I))
