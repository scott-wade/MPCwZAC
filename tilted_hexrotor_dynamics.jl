...
function propeller_to_body(α, β, λ) # rotation matrix from propeller to body frame
    ...
    Rz= [cos(λ), sin(λ), 0; -sin(λ), cos(λ), 0; 0, 0, 1]  
...
function hexrotor_dynamics(model::NamedTuple,x,u)
    ...
    R_propeller2body= [zeros(3, 3) for _ in 1:6]
    rotor_position= [zeros(3, 1) for _ in 1:6]
    for i= 1:6
    ...
    torque_thrust_body= 0
    for i= 1:6
        torque_thrust_body += cross(rotor_position[i], (R_propeller2body * thrust_propeller[i]))
    end
    ...
    torque_drag_body= 0
    for i= 1:6
        torque_drag_body += R_propeller2body[i] * drag_propeller[i]
    end
    ...
    thrust_force_body= 0
    for i= 1:6
        thrust_force_body += R_propeller2body[i] * thrust_propeller[i]
    end

    # total force in world frame
    f = mass*gravity + Q * thrust_force_body # forces in world frame
